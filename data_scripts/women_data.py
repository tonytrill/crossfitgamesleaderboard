...
url = 'https://games.crossfit.com/competitions/api/v1/competitions/open/2019/leaderboards?division=2&' \
...
for i in range(1, totalPages+1):
    ...
    try:
        url = 'https://games.crossfit.com/competitions/api/v1/competitions/open/2019/leaderboards?division=2&region=0&scaled=0&sort=0&occupation=0&page=' + str(
            i)
        response = requests.get(url)
        json_output = response.json()
...
entrants_csv.to_csv('~/Data/athletes_women_2019_'+today+'.csv', index=False)
scores_csv.to_csv('~/Data/scores_women_2019_'+today+'.csv', index=False)

# with open('~/Data/failed_pages_women_'+today+'.csv', 'w') as myfile:
